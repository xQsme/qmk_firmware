...
void keyboard_post_init_user(void) {	//run as last task in keyboard init
	...
	rgblight_sethsv(58, 255, 100); //Set lower default brightness when connected as the right side dimming is not yet in effect
...
layer_state_t layer_state_set_user(layer_state_t state) {
	switch (get_highest_layer(state)) {
		...
		static uint16_t underglow_brightness = 120;
		static uint16_t perkey_brightness = 120;
		static uint16_t lastrow_brightness = 30; //for less shine-through at case edges
